 ...
 * offline. This datapack has one alias per line
 * 
 * @author roi
 * 
 */
...
public class FormatReader {
     ...
     * @param input
     *            input file name
     * @param queryThreshold
     *            minimum number of counts on query logs to store the candidate
     * @param wikiThreshold
     *            minimum number of counts on wiki anchor to store the candidate
     * @return
     */
	...
	return new Iterable<StringAndCandidate>() {
		...
		try {
			...
			LineIterator lineIterator = new LineIterator( new FastBufferedReader( new InputStreamReader( new FileInputStream( input ),
				Charsets.UTF_8 ) ) );
	...
	if ( ents.size() > 0 ) {
		...
		if ( ( e.LAET >= wikiThreshold || e.QAEF >= queryThreshold ) && !e.equals( max ) ) {
		    ...
		    if ( !e.equals( max ) )
			LOGGER.info( "removing " + e + " laet? " + e.LAET + ">= " + wikiThreshold + "?" + ( e.LAET >= wikiThreshold ) + " qaef?"
				+ ( e.QAEF >= queryThreshold ) + " wikT " + wikiThreshold + " qT " + queryThreshold );
		}
	    }
	...
	SimpleJSAP jsap = new SimpleJSAP( FormatReader.class.getName(), "Tests the reading format", new Parameter[] { new FlaggedOption( "input",
		JSAP.STRING_PARSER, JSAP.NO_DEFAULT, JSAP.REQUIRED, 'i', "input", "Input file" ), } );
