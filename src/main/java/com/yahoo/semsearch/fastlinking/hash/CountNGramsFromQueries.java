 ...
 * do this is because the current datapack only counts queries as events, but
 * the model operates on a term by term basis so some of the inner ngrams might
...
public class CountNGramsFromQueries {
    ...
    //TODO We actually don't need the hash for this. Read the possible phrases at the beginning of the process and load into a SignedMinimalPerfectHash  
	...
	//Object2DoubleOpenHashMap<String> counts = new Object2DoubleOpenHashMap<String>( numberOfPhrases );
	long[] counts = new long[ numberOfPhrases ];
	pl.set( numberOfPhrases );
	pl.start( "Starting to count n-grams" );
	for ( StringAndCandidate sc : stringAndCandidates2 ) {
		...
		for ( int j = i; j < words.length; j++ ) {
		    ...
		    if ( l != null ) {
			//if ( f.get( ss ) != null ) {					
			//counts.addTo( ss, sc.candidatesInfo.QAT );
			counts[ l.intValue() ] += sc.candidatesInfo.QAT; //TODO unique per query
		    }
		    sb.append( " " );
		}
	    }
	...
	pl.stop();
	pl.start( "Starting to output new datapack into " + jsapResult.getString( "output" ) );
	...
	for ( StringAndCandidate sc : stringAndCandidates2 ) {
	    fw.write( sc.surfaceForm + "\u0001" + toInt( sc.candidatesInfo.QAF ) + "\u0001" + toInt( sc.candidatesInfo.QAC ) + "\u0001"
		    + counts[ ( (Long) f.get( sc.surfaceForm ) ).intValue() ] + "\u0001" + 1 + "\u0001" + 1 + "\u0001" + toInt( sc.candidatesInfo.LAF )
		    + "\u0001" + toInt( sc.candidatesInfo.LAT ) + "\u0001" + sc.candidatesInfo.entities.length );
	    for ( Entity e : sc.candidatesInfo.entities ) {
		fw.write( "\t" + e.id + "\u0001" + e.type + "\u0001" + toInt( e.QEF ) + "\u0001" + toInt( e.QAEF ) + "\u0001" + "1" + "\u0001" + "1" + "\u0001"
			+ toInt( e.LET ) + "\u0001" + toInt( e.LAET ) );
		//System.out.println(sc.surfaceForm + "\t"+  e.id);
	    }
