...
public class ProbabilityRanker extends CandidateRanker {
	...
	public double rank( Entity e, CandidatesInfo info, EntityContext context, String surfaceForm, int length ) {
		...
		double p_e_l_a = ( e.QAEF + mu_0 * p_e ) / ( info.QAF + mu_0 ); //otherwise ( e.QAEF + 1 ) / (
		// numberOfCandidates + QAF );
		double p_e_l_a_w = ( e.LAET + mu_1 * p_e_w ) / ( info.LAF + mu_1 ); // or ( e.LAET + 1 ) / (
		// numberOfCandidates + LAF )
		if( p_e_l_a_w > 1 ) p_e_l_a_w = 1; //TODO this might happen because there were some bogus counts in old datapacks - it might
		// be ok to remove now
		...
		if( DEBUG ) System.out.println( "\033[1m" + hash.getEntityName( e.id ) + "\033[0m type = " + e.type + " score=" + score + " " + " #Cand=" + info.entities.length + " QAEF=" + e.QAEF + " QEF= " + e.QEF + " QAF=" +
				info.QAF + " QAT=" + info.QAT + " QAC=" + info.QAC + " LAEF= " + e.LAET + " LAF=" + info.LAF + " LAT= " + info.LAT + " " + "tClicks=" + info.totalPriorProbOfClick + " p(e|l,a)=" + p_e_l_a + " p(e|l,a;w)" +
				"=" + p_e_l_a_w + "" + " priorQ = " + priorQ + " priorA " + ( 1 - priorQ ) +
