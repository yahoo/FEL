...
public class ProbabilityRanker extends CandidateRanker {
    ...
    public double pQuery = 0.5;
    public double pAnchor = 1 - pQuery;
    private double mu_0 = 10;
	...
	double p_l_a_w = ( info.LAF + 1 ) / ( info.LAT + 2 );//chance of linking given Wiki 
	double p_l_a = ( info.QAC + 1 ) / ( info.QAT + 2 ); //chance of linking given query
	...
	if ( p_e_l_a_w > 1 ) p_e_l_a_w = 1; //TODO this might happen because there were some bogus counts in old datapacks - it might be ok to remove now
	...
	if ( DEBUG )
	    System.out.println( "\033[1m" + hash.getEntityName( e.id ) + "\033[0m type = " + e.type + " score="
		    + score
		    + " "
		    + " #Cand=" + info.entities.length + " QAEF=" + e.QAEF + " QEF= " + e.QEF + " QAF=" + info.QAF + " QAT=" + info.QAT + " QAC=" + info.QAC
		    + " LAEF= " + e.LAET + " LAF=" + info.LAF + " LAT= " + info.LAT + " " + "tClicks=" + info.totalPriorProbOfClick + " p(e|l,a)=" + p_e_l_a
		    + " p(e|l,a;w)=" + p_e_l_a_w + "" + " priorQ = " + priorQ + " priorA " + (1-priorQ) + " p(l|a) = " + p_l_a + " p(l|a;w) = " + p_l_a_w
		    + " anchorContr " + ( p_l_a_w * (1-priorQ) * p_e_l_a_w ) + " queryContr " + ( p_l_a * priorQ * p_e_l_a ) + " alias [ " + surfaceForm + " ] "
		    );
