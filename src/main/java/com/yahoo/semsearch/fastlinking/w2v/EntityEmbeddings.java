...
public class EntityEmbeddings extends Configured implements Tool {
	...
	while ( ( line = br.readLine() ) != null )
	    count++;
	br.close();
	final PrintWriter pw = new PrintWriter( new BufferedWriter( new FileWriter( jsapResult.getString( "output" ), true ) ) );
	pw.println( count + " " + d );
	int rho = -1; //in the paper we say 20, if this value is negative we use the same number of positive words for negative sampling
	float alpha = 10;
	...
	while ( total < rho ) {
	    ...
	    while ( positiveWords.contains( xx ) )
		xx = r.nextInt( nwords );
	...
	while ( !convergence ) {
		...
		//System.out.println( "w["+j+"]="+ w[j ] + " acum " + acum + " grad " + alpha * ( (1D/N) * ( acum + C * w[ j ] )));
